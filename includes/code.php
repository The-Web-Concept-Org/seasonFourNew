...
<?php
/*Add Channel*/
if (!empty($_POST['action']) and $_POST['action'] == "add_new_branch") {
	...
	$data_user = [
		...
		'branch_status' => $_REQUEST['branch_status'],
	];


	if ($_REQUEST['new_branch_id'] == '') {
...
if (!empty($_REQUEST['branch_del_id'])) {
	...
	mysqli_query($dbc, "UPDATE branch SET branch_status = '0' WHERE branch_id = '$_REQUEST[branch_del_id]'");
	redirect('branch.php', 2000);
}

if (!empty($_REQUEST['branch_edit_id'])) {

	$fetchusers = fetchRecord($dbc, "branch", 'branch_id', $_REQUEST['branch_edit_id']);
	$users_button = ' <button type="submit" id="budget_category" name="branch_edit" data-loading-text="Loading..." class="btn btn-admin2 pull pull-right"> Edit </button>';
} else {
	$users_button = ' <button type="submit" id="budget_category" name="branch_add" data-loading-text="Loading..." class="btn btn-admin pull pull-right">Save </button>';
}

if (isset($_REQUEST['user_branch'])) {

	$user_id = $_REQUEST['branch_edit_id'];
	$data_user_update = [
		'branch_name' => @$_REQUEST['branch_name'],
		'branch_email' => $_REQUEST['branch_email'],
		'branch_phone' => $_REQUEST['branch_phone'],
