                ...
                <table class="table  saleTable" id="myDiv">
                  ...
                  <tbody class="table table-bordered" id="purchase_product_tb">
                    <?php if (isset($_REQUEST['edit_purchase_id'])):
                      $q = mysqli_query($dbc, "SELECT  product.*,brands.*,purchase_return_item.* FROM purchase_return_item LEFT JOIN product ON product.product_id=purchase_return_item.product_id LEFT JOIN brands ON product.brand_id=brands.brand_id   WHERE purchase_return_item.purchase_id='" . base64_decode($_REQUEST['edit_purchase_id']) . "'");
